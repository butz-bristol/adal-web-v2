...
const StudentPersonalInformation = () => {
  ...
  const selectedRegion = regions?.find(
    (r) => r.reg_code === studentData.region?.reg_code
  )?.reg_code;
  const selectedProvince = provinces?.find(
    (p) => p.prov_code === studentData.province?.prov_code
  )?.prov_code;
  const selectedMunicipality = city_mun?.find(
    (m) => m.mun_code === studentData.municipality?.mun_code
  )?.mun_code;
  const departmentName = departments.find(
    (department) => department?._id === studentData.student_department?._id
  )?.department_name;
    ...
    if (e.target.name === 'student_permanent_address' && isPresentAddress) {
      dispatch(
        setStudent({
          ...
          student_present_address: studentData?.student_permanent_address,
        })
      );
    }
  };
  ...
  const handleCheckboxAddress = (e) => {
    const data = { ...studentData };
    if (!studentData?.student_present_address) {
  ...
  const handleChangeNationality = (e, value) => {
    dispatch(setStudent({ ...studentData, student_nationality: value }));
  };
      ...
      <Grid container spacing={2}>
        ...
        <Grid item xs={12}>
          <FormControl>
            ...
            <RadioGroup
              ...
              value={studentData.student_pwd_status ? 'Yes' : 'No' || ''}
            ...
            <Grid item xs={6}>
              <FormControl fullWidth>
                ...
                <Select
                  ...
                  value={studentData.student_sexual_orientation || ''}
                  onChange={handleChange}
                >
            ...
            <Grid item xs={6}>
              <FormControl fullWidth>
                ...
                <Select
                  ...
                  onChange={handleChange}
                  value={studentData.student_civil_status || ''}
                >
        ...
        <Grid item xs={12}>
          <TextField
            ...
            onChange={handleChange}
            value={studentData.student_personal_email || ''}
        ...
        <Grid item xs={6}>
          <Autocomplete
            ...
            isOptionEqualToValue={customIsOptionEqualToValue}
            value={studentData.municipality || null}
            renderInput={(params) => (
              <TextField
                {...params}
                label="Municipality"
        ...
        <Grid item xs={6}>
          <Autocomplete
            ...
            isOptionEqualToValue={customIsOptionEqualToValue}
            value={studentData.barangay || null}
            renderInput={(params) => (
              <TextField
                {...params}
                label="Barangay"
        ...
        <Grid item xs={12}>
          <TextField
            ...
            value={studentData.student_present_address || ''}
            variant="outlined"
            rows={2}
            multiline
          />
        ...
        <Grid item xs={12}>
          {studentData?.student_same_address === false && (
            <TextField
              fullWidth
              type="text"
              ...
              value={studentData.student_permanent_address || ''}
              variant="outlined"
              rows={2}
              multiline
            />
            ...
            <FormControlLabel
              control={
                <Checkbox
                  ...
                  checked={studentData?.student_same_address || false}
                />
              }
