...
const CollegeFeesSetup = () => {
  ...
  const collegeFees = fees?.filter((fee) => fee.applies_to == 'Programs');

  const options = programs
    .filter((program) =>
      program.college_track?.college_track_name?.startsWith('College of')
    )
    .map((program) => {
      return {
        label: `${program?.program_name} - ${program?.program_description}`,
        id: program?._id,
      };
    });

  const filteredPrograms = filter?.label
...
export default CollegeFeesSetup;
