...
const ORSummaryList = () => {
  ...
  const filteredORSummaryList = orSummaryList.filter((orSummary) => {
    if (!orNumber) {
      ...
      return String(orSummary.or_no).includes(orNumber);
    }
  });
  ...
  return (
    <Stack>
      ...
      <ConfirmationModal
        ...
        message={
          'All transactions with this OR No. will be completely deleted. This action cannot be undone.'
        }
        onCancel={() => {
        ...
        onConfirm={() => {
          dispatch(deletePaymentByOr(orNo));
          setShowLastConfirmation(false);
          setShowDeleteConfirmation(false);
          setOrNo('');
        }}
        isOpen={showLastConfirmation}
      />

      <ConfirmationModal
        title="Delete OR"
        message={
          'Kindly confirm if this OR No has been cancelled before proceeding.'
        }
        onCancel={() => {
          setShowDeleteConfirmation(false);
          setShowLastConfirmation(false);
                      ...
                      <TableCell>
                        <Stack direction="row">
                          ...
                          {user_role === 'finance admin' && (
                            <Tooltip title="Delete Transaction(s)">
                              <IconButton
                                color="secondary"
                                onClick={() => {
                                  setOrNo(orSummary.or_no);
