...
import { setMenuId, toggleMenu } from 'src/store/customizationSlice';
...
const NavItem = ({ item, level }) => {
  ...
  const { isOpen, borderRadius } = useSelector((state) => state.customization);
  ...
  const itemHandler = (id) => {
    ...
    if (matchesSM) dispatch(toggleMenu());
  };
  ...
  return (
    <ListItemButton
      ...
      sx={{
        ...
        backgroundColor: level > 1 ? 'transparent !important' : 'transparent',
        py: level > 1 ? 1 : 1.25,
        pl: `${level * 24}px`,
      }}
